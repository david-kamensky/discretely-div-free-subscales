...
print("Generating the Taylor-Hood element.")
...
tau_M_1 = h / 2*verti(a)
tau_M_2 = (h*h) / (C_I*nu)
#tau_M = min([tau_M_1,tau_M_2]) #Commented out because "UFL conditions cannot be evaluated as bool in a python context" error.
tau_M = tau_M_1                 ##Code works fine if tau_M_i are selected manually.

# Define tau_C:
print("Generating stabilization parameter tau_C.")
tau_C_1 = h*verti(a)
tau_C_2 = nu
#tau_C = max([tau_C_1,tau_C_2]) #Commented out because "UFL conditions cannot be evaluated as bool in a python context" error.
tau_C = tau_C_1                 #Code works fine if tau_C_i are selected manually.
...
output_file = open('copypasta-ldcavity-Oseen-TH.txt','w')
